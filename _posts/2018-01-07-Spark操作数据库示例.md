...
/**
  * Created by XX on 2017/4/19.
  */
object MysqlUtil extends Serializable {

  val url = "jdbc:mysql://IP:port/raviz"
  val user = "user"
  val password = "password"
  val driver = "com.mysql.jdbc.Driver"

  /**
    * @param sqlContext 从数据库中读入访问记录
    */
  def getVisitFromMysql(sqlContext: SQLContext) = {
    val jdbcDF = sqlContext.read.format("jdbc").options(Map("url" -> url, "driver" -> driver, "dbtable" -> "visit", "user" -> user, "password" -> password)).load()
    jdbcDF.registerTempTable("visit")
    sqlContext.sql("select uid, rid from visit").rdd.map(row => (row.getAs[String]("uid"), row.getAs[String]("rid"))).distinct()
  }

  /**
    * 从mysql读入用户数据
    **/
  def getUserFromMysql(sqlContext: SQLContext) = {
    val jdbcDF = sqlContext.read.format("jdbc").options(Map("url" -> url, "driver" -> driver, "dbtable" -> "user", "user" -> user, "password" -> password)).load()
    jdbcDF.registerTempTable("user")
    sqlContext.sql("select uid, unation, username from user").rdd.map(row => (row.getAs[String]("uid"), row.getAs[String]("unation"), row.getAs[String]("username")))
  }
}
```
...
object rank {
  ...
  def main(args: Array[String]) {
    ...
    /**
     *
     * @param sqlContext 从数据库中读入表url,与urllist两列
     * @return 源url,终url
     */
    ...
    def saveAllVertex(rank: VertexRDD[Double], graphV: VertexRDD[String]) = {
      ...
      vertexDataFrame.write.mode("append").jdbc("jdbc:mysql://IP:port/lib?useUnicode=true&characterEncoding=UTF-8", "XX", prop)
    ...
    def saveTop500Edge(rank: VertexRDD[Double], graphE: EdgeRDD[Double]) = {
      ...
      prop.put("driver", "com.mysql.jdbc.Driver")
      //将数据追加到数据库
      Top500EdgeDataFrame.write.mode("append").jdbc("jdbc:mysql://202.117.16.188:3306/search_engine?useUnicode=true&characterEncoding=UTF-8", "XX", prop)
      }
...
/**
  * Oracle数据库存取工具
  *
  * Created by XX on 2017/4/1.
  */
object OracleDBUtil {

  val url = "jdbc:oracle:thin:@IP:port:name"
  val user = "user"
  val password = "pwd"
  ...
  /**
    * 获取发票数据
    **/
  def getFpFromOracle(hiveContext: HiveContext) = {
    hiveContext.read.format("jdbc").options(dbstring + (("dbtable", "XX"))).load()
  }

  /**
    * 获取纳税人信息
    **/
  def getNsrSimpleFromOracle(hiveContext: HiveContext) = {
    hiveContext.read.format("jdbc").options(dbstring + (("dbtable", "XX"))).load()
  }
  ...
  /**
    * 获取发票汇总数据（包含行业信息）
    **/
  def getFpXfLojFromOracle(hiveContext: HiveContext) = {
    hiveContext.read.format("jdbc").options(dbstring + (("dbtable", "XX"))).load()
  }
  ...
  /**
    * 将行业税负平均值保存到数据库中
    **/
  def saveHySfAveToOracle(rdd: RDD[(String, Int, Int, Double)], sqlContext: SQLContext) = {

    val schema = StructType(
      List(
        StructField("HY_DM", StringType, true),
        StructField("NSR_COUNT", IntegerType, true),
        StructField("MONTH_COUNT", IntegerType, true),
        StructField("SF", DoubleType, true)
      )
    )

    val rowRDD = rdd.map(x => Row(x._1, x._2, x._3, x._4))
    val dataFrame = sqlContext.createDataFrame(rowRDD, schema)

    JdbcUtils.saveTable(dataFrame, url, "XX", properties)
  }

  /**
    * 将发票购方汇总结果保存到数据库中
    **/
  def saveFpGfSumToOracle(rdd: RDD[(String, (((Int, Int), Double), Long))], sqlContext: SQLContext) = {

    val schema = StructType(
      List(
        StructField("NSRDZDAH", LongType, true),
        StructField("NSRSBH", StringType, true),
        StructField("YEAR", IntegerType, true),
        StructField("MONTH", IntegerType, true),
        StructField("JXSE", DoubleType, true)
      )
    )

    val rowRDD = rdd.map(x => Row(x._2._2, x._1, x._2._1._1._1, x._2._1._1._2, x._2._1._2))
    val dataFrame = sqlContext.createDataFrame(rowRDD, schema)
  ...
  /**
    * 将发票销方汇总结果保存到数据库中
    **/
  def saveFpXfSumToOracle(rdd: RDD[(String, (((Int, Int), (Double, Double, Int)), Long))], sqlContext: SQLContext) = {
    ...
    val schema = StructType(
      List(
        ...
        StructField("YEAR", IntegerType, true),
        StructField("MONTH", IntegerType, true),
        StructField("XXSE", DoubleType, true),
        StructField("SRJE", DoubleType, true),
        StructField("FPSL", IntegerType, true)
      )
    )

    val rowRDD = rdd.map(x => Row(x._2._2, x._1, x._2._1._1._1, x._2._1._1._2, x._2._1._2._1, x._2._1._2._2, x._2._1._2._3))
    val dataFrame = sqlContext.createDataFrame(rowRDD, schema)
...
```
需要用到的jar包 ojdbc14-10.2.0.4.0.jar
